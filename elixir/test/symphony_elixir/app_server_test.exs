defmodule SymphonyElixir.AppServerTest do
                 ...
                 payload["id"] == 2 and
                   case get_in(payload, ["params", "dynamicTools"]) do
                     tools when is_list(tools) ->
                       ...
                       "linear_graphql" in names and "sync_workpad" in names and linear_ok? and workpad_ok?
  ...
  test "app server captures codex side output and logs it through Logger" do
    ...
    try do
      ...
      assert log =~
               "Codex turn stream output for issue_id=issue-stderr issue_identifier=MT-92 session_id=thread-92-turn-92: warning: this is stderr noise"
    after
      File.rm_rf(test_root)
    end
  end
