defmodule SymphonyElixir.OrchestratorStatusTest do
  ...
  test "status dashboard skips offline marker when stdout is not a tty" do
    rendered =
      ExUnit.CaptureIO.capture_io(fn ->
        assert :ok = StatusDashboard.render_offline_status_for_test(fn -> false end)
      end)
  ...
  test "status dashboard renders offline marker when stdout is a tty" do
    rendered =
      ExUnit.CaptureIO.capture_io(fn ->
        assert :ok = StatusDashboard.render_offline_status_for_test(fn -> true end)
      end)
  ...
  test "status dashboard coalesces rapid updates to one render per interval" do
    ...
    assert_receive {:render, first_render_ms, _content}, 2_000
    ...
    assert_receive {:render, second_render_ms, _content}, 2_000
