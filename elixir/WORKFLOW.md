...
- **Delegation is an execution gate, not a suggestion.** After the main model has made the plan, the first bounded search, implementation, test, log-inspection, or other worker step that matches the rules below MUST be an `Agent` call. Completing a qualifying step inline without first delegating it violates this workflow.
...
- `sonnet` — the default worker: implementing an already-decided change, writing tests, running the validation gate and reporting failures verbatim, PR-feedback sweeps, mechanical `land`/merge follow-through.
...
- Keep on the main model, never delegate: the deliverable determination, the plan and its self-review, reproduction-signal design, ambiguous ticket interpretation, architectural or cross-cutting decisions, state moves and workpad/handoff wording, and anything whose wrong call is expensive to undo.
