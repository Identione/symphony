...
1. Open the workpad: search active/unresolved comments for `## Symphony Workpad`; reuse it if present, otherwise create exactly one. Persist its comment ID and write all progress only to that ID. Edit a local `workpad.md` at the workspace root (use the absolute path, e.g. `/path/to/workspace/workpad.md`) and push it with the `sync_workpad` tool — first sync omits `comment_id` and returns `comment.id` (persist it); later syncs pass that id. Keep `## Symphony Workpad` as the first heading so the marker still resolves.
2. Reconcile before new edits: for each plan item, verify against the workspace before marking done — check for existing files (`ls`/`find`), run existing test suites, inspect `git diff --stat`. Check off every item that is verifiably complete, call `sync_workpad` immediately to record the reconciled state, then expand/fix the plan for remaining scope and confirm `Acceptance Criteria` and `Validation` still fit.
...
6. Implement against the TODOs. After ticking off **each individual checkbox**, immediately edit `workpad.md` and call `sync_workpad` — do not batch updates across multiple items or defer to end-of-turn. Never leave completed work unchecked. For a `Todo` ticket that already had a PR attached, run the **PR feedback sweep protocol** before new feature work.
