...
- `Codex.DynamicTool`: emit one `Logger.info` `linear_graphql_call …` line per raw `linear_graphql` tool call (both the Codex and Claude adapters route through it). This is operation **telemetry**, not a payload dump — it records the operation *shape* so the recurring operations can be mined over time to decide which typed, parameterized helpers to build. Format:
  - `linear_graphql_call op_type=<query|mutation|subscription|unknown> root_field=<field> var_keys=<comma,sep,keys> status=<ok|graphql_error codes=…|error http=…|error reason=…|rejected reason=…> [issue_id=… issue_identifier=… session_id=…]`
  - Only the operation type, first root field, variable *keys*, and outcome are logged — never the GraphQL document body or variable *values* (which can be large and carry issue content). `var_keys` is capped at 200 chars. Issue/session context is appended when the caller threads `log_context`. Mine it with e.g. `grep -oE 'root_field=[A-Za-z0-9_]+' symphony.log | sort | uniq -c | sort -rn`.
