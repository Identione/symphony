defmodule SymphonyElixir.StatusDashboard do
  ...
  # Runs outside any GenServer state (it is called from `Application.stop/1`),
  # so it has no `enabled`/`tty_probe` to read. Gate it directly on the same
  # defensive tty check the state-based path uses, so a detached daemon's
  # shutdown does not append an ANSI frame either. `:tty_probe` and
  # `:render_fun` are injectable so both branches are assertable in tests.
