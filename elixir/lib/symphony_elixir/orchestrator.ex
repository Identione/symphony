defmodule SymphonyElixir.Orchestrator do
      ...
      code ->
        ...
        case DeterministicFailure.decide(prev_entry, code, settings) do
          ...
          {updated_entry, action} ->
            next_state = put_deterministic_failure(state, issue_id, updated_entry)
            apply_deterministic_action(next_state, issue_id, running_entry, action, settings)
        end
    end
  end

  defp apply_deterministic_action(state, _issue_id, _running_entry, :no_action, _settings) do
    {:continue, state}
  end

  defp apply_deterministic_action(state, issue_id, running_entry, action, settings) do
    ...
    case DeterministicFailure.handle(action, issue, settings) do
      ...
      {:error, _reason} ->
        # Logged by DeterministicFailure.handle/3. Keep retrying via the
        # normal retry loop so the comment/move can be re-attempted next
        # failure.
        case action do
          {:escalate, _code, _count} ->
            # If the comment posted but the state move failed, retry both on
            # the next failure by clearing the escalation flag.
            {:continue, mark_escalation_pending(state, issue_id)}
  ...
  defp mark_escalation_pending(state, issue_id) do
    case Map.get(state.deterministic_failures, issue_id) do
      %{} = entry ->
        put_deterministic_failure(state, issue_id, %{entry | notified_escalation?: false})
