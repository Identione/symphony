defmodule SymphonyElixir.DeterministicFailure do
  ...
  *deterministic* subset feeds the counter — codes whose recurrence implies a
  ...
  The `:notified_alert?` / `:notified_escalation?` flags are pre-set on the
  returned entry so the orchestrator stores them verbatim and the next
  failure with the same code won't re-trigger the same action.
  """
  ...
  def decide(prev_entry, code, settings) when is_atom(code) do
    if deterministic?(code) do
      ...
      classify_with_mutation(incremented, settings)
    else
      {:drop, :no_action}
    end
  end
  ...
  * On `:escalate` — emit the same comment (idempotent if the alert path
  ...
  defp classify_with_mutation(entry, settings) do
    ...
    cond do
      is_integer(escalate) and count >= escalate and not entry.notified_escalation? ->
        # Escalation marks `notified_alert?` too so a streak that crosses both
        # thresholds in one decision (e.g. fresh failure with N=1 and M=1)
        # emits exactly one comment + state-move rather than alert+escalate.
        {%{entry | notified_escalation?: true, notified_alert?: true}, {:escalate, entry.code, count}}

      is_integer(alert) and count >= alert and not entry.notified_alert? ->
        {%{entry | notified_alert?: true}, {:alert, entry.code, count}}
