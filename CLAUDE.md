...
> **`--require-label` / `--exclude-label` on `make init`:** each flag is repeatable. It sets `tracker.required_labels` / `tracker.excluded_labels` in the generated workflow. An issue is picked up only if it carries every `required_labels` entry and none of the `excluded_labels` entries. Label matching is case-insensitive, so the same label in both lists is rejected at generation time (it can never be satisfied — the daemon would boot, poll, and claim nothing). Whichever list stays empty is emitted as a commented example, so the unused half of the gate stays discoverable; omitting both flags is the default and the generated workflow carries the full commented example instead of an active gate.
